...
test('Pagination reducer -> feedErrorReducer -> REQUEST_FEED', () => {
...
test('Pagination reducer -> feedErrorReducer -> SUCCESS_RECEIVE_FEED', () => {
  const output = PaginationReducerFactory({}, { type: "SUCCESS_RECEIVE_FEED", payload: {feedName: FEED_NAME}, meta: { direction: "initial", endpoint: 'api'}})
  expect(output).toMatchObject( { [FEED_NAME]: { below: { isFetching: false } } });
});

test('Pagination reducer -> feedErrorReducer -> ERROR_RECEIVE_FEED', () => {
...
test('Pagination reducer -> feedErrorReducer -> UPDATE_PAGINATION', () => {
